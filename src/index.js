...
function filterByCity(city) {
  ...
  for (let i = 0; i < window.contacts.length; i++) {
    if (window.contacts[i].address.city === city) {
        resultArray.push(window.contacts[i]);
    }
  }
  return resultArray;
}
...
function filterHandler() {
  const filterOps = document.querySelector("#filterOptions");
    filterOps.addEventListener("change", (event) => {
      event.preventDefault();
  const changedState = document.getElementById("filterOptions").value;
  if (changedState === 0){
    render(window.contacts);
  } else {
  const cityArray = filterByCity(changedState);
    render(cityArray);
  }
  })
}
...
function loadCities(contacts) {
  ...
  for (var i = 0; i < contacts.length; i++) {
      if (!resultArray.includes(contacts[i].address.city)) {
        resultArray.push(contacts[i].address.city)
      }
 }
...
function deleteButtonHandler(event) {
  const allBtns = document.querySelectorAll(".deleteBtn");
  
  allBtns.forEach((btn) => {
    btn.addEventListener("click", (event) => {
     event.preventDefault(); 
      const clickedDeleteBtnId = event.target.parentElement.getAttribute("data-id");
      deleteContact(clickedDeleteBtnId);
      render(window.contacts);
    });
  });
